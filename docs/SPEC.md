...
- Use the gold X icon consistently for the application bundle and Dock, with transparent outer margins and rounded corners so the artwork does not fill the entire Dock icon slot. Preserve the original official-site asset and record its source, the generated derivative, and its exact generation prompt. No hotlinked icon dependency at runtime.
